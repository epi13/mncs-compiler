...
| ASCII lexical analysis (tokens, keywords, operators, comments, diagnostics) | full | **parity with known exceptions**: full kind/span/diagnostic parity on 196-sample corpus; exceptions: profile-gated `!`→`not` (CP-0015), Unicode policy (CP-0002) | frontend + segment suites; `evidence/frontend-results.json` |
...
| Types / signatures / contracts / effects | full (MNE105/MNE110/MNE111/...) | **partial, blocked**: signature facts + per-fn gates at old pin | `evidence/sem-results.json` (old pin, 49 cases) |
